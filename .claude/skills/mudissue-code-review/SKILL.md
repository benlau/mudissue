...
Read [docs/dev/module-layers.md](../../../docs/dev/module-layers.md) first. Layer rules are about **where behavior lives**, not only import direction (views may import `utils/storage`; that does not justify new helpers in the wrong folder).
...
| -------- | ----------------------------- |
| Does this touch issue/repo files or frontmatter? | Belongs in L2 `src/utils/storage/` (e.g. `TrackerRepoStorage`, `IssueFolderStorage`, `IssueMarkdownFileStorage`), not in `views/` or palette-local helpers. |
...
**Why wrong:** That file holds **command/workflow** behavior (do this step after set status), not a **utility** that belongs under `views/`. Palette commands should orchestrate; persistence belongs on `IssueFolderStorage` in `utils/storage/`. A new helper class in the wrong folder breaks architecture even when imports are valid.
