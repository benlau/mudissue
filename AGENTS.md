...
| Async          | `src/async/`           | Async utilities (storage, validators, search, launchers, etc.)                                                          |
...
- `src/helpers/`: shared orchestration at L4. Use for workflows that combine storage, stores, and services—e.g. `NextIssueIdHelper` (allocate/resolve issue IDs), `CreateIssueHelper` (TUI create/subissue), `ChangeIssueIdHelper`, `CurrentIssueResolverHelper`. Do **not** place helpers under `src/async/storage/` to bypass layer rules.
...
- `src/async/`: async utility modules. May depend on services, but must not depend on store modules (this restriction applies to `src/async`, not to `src/commands`).
...
- **Store-owned behavior:** Methods that read or interpret state held by a store belong on that store (e.g. `getSelectedIssues()`, `getCurrentPage()`), not as parallel helpers elsewhere. Callers use the store via `getState()` or hooks—do not reimplement the same logic in views, commands, or async.
...
- Tracker repo discovery (`find`, `findSubTrackerRepos`) and config loading live in `TrackerRepoStorage` (`src/async/storage/`).
