...
- `src/helpers/`: shared orchestration at L4. Use for workflows that combine storage, stores, and services—e.g. `NextIssueIdHelper` (allocate/resolve issue IDs), `CreateIssueHelper` (TUI create/subissue), `ChangeIssueIdHelper`, `CurrentIssueResolverHelper`. Do **not** place helpers under `src/utils/storage/` to bypass layer rules.
